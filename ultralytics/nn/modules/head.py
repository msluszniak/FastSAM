...
class Detect(nn.Module):
    ...
    anchors = torch.zeros((2, 21504))
    strides = torch.zeros((1, 21504))
    ...
    def forward(self, x):
        ...
        if self.training:
            return x
        elif self.dynamic or self.shape_h != shape[-2] or self.shape_w != shape[-1]:
            # print([a.shape for a in x])
            # self.anchors, self.strides = (x.transpose(0, 1) for x in make_anchors(x, self.stride, 0.5))
            # print(self.anchors.shape)
            # self.anchors = torch.zeros((2, 21504))
            # self.strides = torch.zeros((1, 21504))
            make_anchors(x, self.stride, self.anchors, self.strides)
            # self.anchors.transpose(0, 1)
        ...
        # print(dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), x, xywh=True, dim=1).shape)
        # print(self.anchors.shape)
        # print(dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1).shape)
        # print(self.strides.shape)
